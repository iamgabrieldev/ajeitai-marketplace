"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { useAuth } from "@/providers/auth-provider";
import { clientesApi, type DadosCadastroCliente, type DadosEndereco } from "@/lib/api";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Wrench, ArrowRight } from "lucide-react";

function onlyDigits(s: string) {
  return s.replace(/\D/g, "");
}

export default function ClienteWelcomePage() {
  const { user, token } = useAuth();
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [step, setStep] = useState(0);
  const [form, setForm] = useState({
    telefone: "",
    cpf: "",
    logradouro: "",
    bairro: "",
    cep: "",
    numero: "",
    complemento: "",
    cidade: "",
    uf: "",
  });

  const handleStart = () => setStep(1);

  const buildEndereco = (): DadosEndereco => ({
    logradouro: form.logradouro.trim(),
    bairro: form.bairro.trim(),
    cep: onlyDigits(form.cep).slice(0, 8),
    cidade: form.cidade.trim(),
    uf: form.uf.trim().toUpperCase().slice(0, 2),
    numero: form.numero.trim(),
    ...(form.complemento.trim() && { complemento: form.complemento.trim() }),
  });

  const handleVincular = async () => {
    if (!token || !user?.firstName || !user?.email) return;
    const nome = [user.firstName, user.lastName].filter(Boolean).join(" ") || user.firstName;
    const endereco = buildEndereco();
    const payload: DadosCadastroCliente = {
      nome,
      email: user.email,
      telefone: onlyDigits(form.telefone).slice(0, 11),
      cpf: onlyDigits(form.cpf),
      endereco,
    };
    setLoading(true);
    try {
      await clientesApi.vincular(token, payload);
      router.replace("/cliente/home");
    } catch (err) {
      console.error("Erro ao vincular:", err);
    } finally {
      setLoading(false);
    }
  };

  const updateField = (field: string, value: string) => {
    setForm((prev) => ({ ...prev, [field]: value }));
  };

  const isValid =
    form.telefone.trim().length >= 10 &&
    form.cpf.replace(/\D/g, "").length >= 11 &&
    form.logradouro.trim() &&
    form.bairro.trim() &&
    onlyDigits(form.cep).length === 8 &&
    form.numero.trim() &&
    form.cidade.trim() &&
    form.uf.trim().length === 2;

  if (step === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6">
        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-primary/10">
            <Wrench className="h-10 w-10 text-primary" />
          </div>
          <h1 className="text-2xl font-bold text-text dark:text-text-dark">
            Bem-vindo ao Ajeita<span className="text-primary">i</span>!
          </h1>
          <p className="max-w-xs text-text-muted dark:text-text-dark-muted">
            Olá, {user?.firstName}! Encontre os melhores prestadores de serviços
            domésticos perto de você.
          </p>
        </div>
        <Button onClick={handleStart} size="lg" className="w-full max-w-xs">
          Começar
          <ArrowRight className="h-5 w-5" />
        </Button>
      </div>
    );
  }

  return (
    <div className="flex min-h-screen flex-col px-6 py-8">
      <h1 className="mb-2 text-xl font-bold text-text dark:text-text-dark">
        Complete seu cadastro
      </h1>
      <p className="mb-6 text-sm text-text-muted dark:text-text-dark-muted">
        Precisamos de algumas informações para melhorar sua experiência.
      </p>

      <div className="flex flex-col gap-4">
        <Input
          label="Telefone"
          placeholder="(00) 00000-0000"
          value={form.telefone}
          onChange={(e) => updateField("telefone", e.target.value)}
        />
        <Input
          label="CPF"
          placeholder="000.000.000-00"
          value={form.cpf}
          onChange={(e) => updateField("cpf", e.target.value)}
        />
        <Input
          label="CEP"
          placeholder="00000-000"
          value={form.cep}
          onChange={(e) => updateField("cep", e.target.value)}
        />
        <Input
          label="Logradouro"
          placeholder="Rua, avenida..."
          value={form.logradouro}
          onChange={(e) => updateField("logradouro", e.target.value)}
        />
        <Input
          label="Número"
          placeholder="Nº"
          value={form.numero}
          onChange={(e) => updateField("numero", e.target.value)}
        />
        <Input
          label="Bairro"
          placeholder="Bairro"
          value={form.bairro}
          onChange={(e) => updateField("bairro", e.target.value)}
        />
        <Textarea
          label="Complemento"
          placeholder="Apto, bloco, etc."
          value={form.complemento}
          onChange={(e) => updateField("complemento", e.target.value)}
        />
        <div className="grid grid-cols-2 gap-4">
          <Input
            label="Cidade"
            placeholder="Sua cidade"
            value={form.cidade}
            onChange={(e) => updateField("cidade", e.target.value)}
          />
          <Input
            label="Estado"
            placeholder="UF"
            value={form.uf}
            onChange={(e) => updateField("uf", e.target.value.toUpperCase().slice(0, 2))}
          />
        </div>
      </div>

      <div className="mt-8">
        <Button
          onClick={handleVincular}
          loading={loading}
          disabled={!isValid}
          size="lg"
          className="w-full"
        >
          Salvar e continuar
        </Button>
      </div>
    </div>
  );
}
